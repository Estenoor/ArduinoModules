...
void initialize(int ARDUINO_ADDR, long int SERIAL_BAUD)
{
  ...
  //RoboRio I2C Code
  Wire.begin(ARDUINO_ADDR); //This makes the arduino an i2c slave device with an address of 0x73
...
void lidarServo(char writeAdress, char readAddress, byte dataArray[2], bool moniterBusy, char lidarliteAddress)
{
  ...
  {
    Write(writeAdress, 0x04, lidarliteAddress);
    Rread(readAddress, 2, dataArray, moniterBusy, lidarliteAddress);
    ...
    int lengthofBusy = Busy(lidarliteAddress);
    servo.write(degree);
    x++;
    z += 2;
    a += 2;
  ...
  {
    Write(writeAdress, 0x04, lidarliteAddress);
    Read(readAddress, 2, readData, moniterBusy, lidarliteAddress);
    dataToSend[z] = readData[0];
    dataToSend[a] = readData[1];
    lidarData[highArray][x] = readData[0];
    lidarData[lowArray][x] = readData[1];
