  ...
  void setup() 
  {
   ...
   Serial.begin(9600); // Initialize Serial Communication at 9600 bits per secon
...
/*
    ...
    theaterChase(strip.Color(  0,   0,   0), 50); // Green
    ...
    rainbow(50);
    rainbowCycle(50);  // make it go through the cycle fairly fast
...
*/    
  ...
  void sam_1()
  {
   ...
   for (i=0; i<32; i++)
      {
       strip.setPixelColor(l--, strip.Color(0, 127, 127)); 
       strip.setPixelColor(r++, strip.Color(127, 0, 127));
       strip.show();
       delay(50);
      }
    for(i=0; i<64; i++)
  ...
  void sam_2()
  {
    ...
    strip.setPixelColor(r, strip.Color(0, 0, 127));
    strip.show();
    delay(200);
  ...
  void sam_3()
  {
    ...
    if (direction == false)
      {
        for (i=0; i<64; i++)
           {
            strip.setPixelColor(f, strip.Color(0, 127, 0));
            strip.show();
            delay(200);
    ...
    if (direction == true)
    {
       for (i=0; i<64; i++)
       {
            strip.setPixelColor(b, strip.Color(0, 127, 0));
            strip.show();
            delay(200);
...
void sam_4(uint8_t wait)
{
 int i;                                 // Clock Variable
 int n = 0;                             // Remainder Variable
 int c = 0;                             // Unknown for now
 int d = 0;                             // Variable for pixel to find remainder
 uint32_t j;                                 // Variable for switching color
 uint32_t r;                                 // Unknown for now
 unin32_t p = strip.Color(50, 0, 50);        // Color is Purple
 unin32_t b = strip.Color( 0, 0, 50);        // Color is Blue

 for (i=0; i<5; i++)
 {
  n = d % 5; // if the remainder == 0 then it toggles color 
  if (n == 0)
  {
   
   j = strip.getPixelColor(0);
   Serial.println(j);
    
   if (j == p)
   {
    r = strip.Color( 0, 0, 50);
   }
   else if (j == b)
   {
    r = strip.Color(50, 0, 50);
   }
